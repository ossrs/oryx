...
if [[ ! -d srs-cloud ]]; then git clone https://github.com/ossrs/srs-cloud.git; fi &&
...
if [[ -f ~/Downloads/aapanel/$PLUGIN.zip ]]; then
  mv $TMP_DIR/$PLUGIN.zip ~/Downloads/aapanel/$PLUGIN.zip
  echo "Plugin OK:"
  echo "    Zip moved to ~/Downloads/aapanel/$PLUGIN.zip"
else
  echo "You could move it by:"
  echo "    mv $TMP_DIR/$PLUGIN.zip ~/Downloads/aapanel/$PLUGIN.zip"
fi
