...
func options() string {
	return fmt.Sprintf("log=%v, timeout=%vms, secret=%vB, endpoint=%v, waitReady=%v, initPassword=%v",
		*srsLog, *srsTimeout, len(*apiSecret), *endpoint, *waitReady, *initPassword)
}
...
func waitForServiceReady() {
	ctx, cancel := context.WithTimeout(logger.WithContext(context.Background()), time.Duration(*apiReadyimeout)*time.Millisecond)
